...
# ===================================================================
# Outflows (Saídas) Endpoints
# ===================================================================

@router.post("/outflows", response_model=schemas.PDVOutflow, status_code=201)
def create_outflow(
    data: schemas.PDVOutflowCreate,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """Regista uma saída de produto (uso interno) ou de dinheiro (sangria de caixa)."""
    return controller.create_outflow(db, data, current_user.id)
...
def get_outflows_summary(
    ...
    """Retorna um resumo das saídas (contagens e totais) para o período dado."""
    return controller.get_outflows_summary(db, current_user.id, start_date, end_date)


@router.get("/outflows", response_model=List[schemas.PDVOutflow])
def list_outflows(
    outflow_type: Optional[str] = Query(None, description="'product' ou 'cash'"),
    start_date: Optional[datetime] = Query(None),
    end_date: Optional[datetime] = Query(None),
    skip: int = Query(0, ge=0),
    limit: int = Query(100, ge=1, le=500),
    ...
    """Lista todas as saídas do terminal, com filtros opcionais."""
    return controller.get_outflows(db, current_user.id, outflow_type, start_date, end_date, skip, limit)
...
def cancel_outflow(
    ...
    """Cancela uma saída e reverte o stock (se for produto) ou o levantamento (se for dinheiro)."""
    return controller.cancel_outflow(db, outflow_id, current_user.id)
