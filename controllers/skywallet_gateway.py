...
class SkyWalletGatewayClient:
    ...
    async def get_balance(self, central_user_id: str, user_details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                raise HTTPException(status_code=response.status_code, detail="Failed to get balance from SkyWallet")
    ...
    async def charge(self, user_details: Dict[str, Any], amount: float, reference: str, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                raise HTTPException(status_code=response.status_code, detail="Failed to charge user from SkyWallet")
            return response.json()
    
    async def deposit(self, user_details: Dict[str, Any], amount: float, payment_method: str = "mpesa") -> Dict[str, Any]:
        # This would handle deposit flow
        raise NotImplementedError("SkyWallet deposit endpoint not yet integrated")
